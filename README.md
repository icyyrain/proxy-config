...
- `🏰 Disney+`：使用 ACL4SSR 的 DisneyPlus 规则，可独立选择线路。
- `🎬 小宝影院`：网站及常见 `dytt-` 视频源域名默认使用香港节点，避免自建美国线路到片源速度过低。
- `🎮 PS5节点`：供自定义 PS5 规则使用，默认跟随节点选择，也可在自建、香港、日本、美国节点与直连之间独立选择。
- 18 个业务与媒体分组可直接选择 `🏠 自建节点` 或 `🛟 备用自建节点`，无需改变全局节点选择。
